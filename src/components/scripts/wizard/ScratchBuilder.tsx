...
type Step = { order: number; name: string; goal: string; duration_estimate_minutes: number; script_lines: string[]; questions: string[]; tips: string[]; key_phrases: string[] };
...
export function ScratchBuilder() {
  ...
  const [steps, setSteps] = useState<Step[]>([{ order: 1, name: "", goal: "", duration_estimate_minutes: 5, script_lines: [], questions: [], tips: [], key_phrases: [] }]);
  const [objections, setObjections] = useState<Objection[]>([{ order: 1, label: "", category: "price", responses: [], key_reframe: "", trigger_phrases: [], applicable_step_orders: [] }]);
  ...
  function addStep() { setSteps(s => [...s, { order: s.length + 1, name: "", goal: "", duration_estimate_minutes: 5, script_lines: [], questions: [], tips: [], key_phrases: [] }]); }
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/scripts/save", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ script: { name, goal, duration_minutes: duration, reminders, steps, objections } }) });
      ...
      <div className="flex-1 min-w-0 space-y-6 pb-8">
        <section className="border border-stone-200 rounded-xl p-5 space-y-4 bg-white">
          ...
          <div className="space-y-1.5">
            ...
            <Input value={goal} onChange={e => setGoal(e.target.value)} placeholder="Ex: Closer le prospect sur l'offre à 2000€" className="h-9 text-sm border-stone-200" />
          </div>
          <div className="space-y-1.5">
